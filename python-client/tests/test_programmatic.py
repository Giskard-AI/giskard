...
def test_a_greater_b_fail():
    passed, _ = Suite().add_test(_test_a_greater_b.set_params(1, 2)).run()
    assert not passed


def test_a_greater_b_pass():
    passed, _ = Suite().add_test(_test_a_greater_b.set_params(2, 1)).run()
    assert passed


def test_missing_arg():
    with pytest.raises(Exception, match="Missing 1 required parameters: {'b': <class 'int'>}"):
        Suite().add_test(_test_a_greater_b.set_params(a=2)).run()
...
def test_multiple(german_credit_data: Dataset, german_credit_model: BaseModel):
    assert (
        Suite()
        .add_test(test_auc.set_params(threshold=0.2))
        .add_test(test_f1.set_params(threshold=0.2))
...
def test_shared_input(german_credit_model: BaseModel, german_credit_data: Dataset):
    ...
    assert (
        ...
        .add_test(test_f1.set_params(dataset=shared_input, threshold=0.2))
        .add_test(test_diff_f1.set_params(threshold=0.2, actual_dataset=shared_input))
        .run(
...
def test_save_suite(german_credit_data: Dataset, german_credit_model: BaseModel):
    with MockedClient() as (client, mr):
        Suite().add_test(test_auc.set_params(threshold=0.2, dataset=german_credit_data)).add_test(
            test_f1.set_params(threshold=0.2, dataset=german_credit_data)
