...
In the following example, we illustrate the procedure using **OpenAI** and **Azure OpenAI**; however, please note that our platform supports a variety of language models (e.g., OpenAI, Azure OpenAI, Ollama, Mistral). For details on configuring different models, visit our [Setting Up Language Models page](#)
...
> #### ℹ️ What data are being sent to Lamguage Model Providers
> 
> In order to perform tasks with language model-assisted detectors, we send the following information to the selected language model provider (e.g., OpenAI, Azure OpenAI, Ollama, Mistral):
> 
