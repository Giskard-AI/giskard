...
- `metadata`: a dictionnary with various metadata about the question, this includes the `question_type`, `seed_document_id` the id of the document used to generate the question and the `topic` of the question

(q_types)=
## Question types
RAGET currently proposes 6 question types. Each of them is designed to evaluate specific components of the RAG system. 
```{list-table}
:header-rows: 1
:widths: 20, 50, 25
* - Question type
