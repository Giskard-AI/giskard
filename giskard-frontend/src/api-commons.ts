...
export function dialogDownloadFile(response: AxiosResponse, fileName: string) {
  ...
  fileLink.href = fileURL;
   const fileNameFromHeader = response.headers['content-disposition'].match(/filename=(.+)/);
   fileName=(fileNameFromHeader && fileNameFromHeader.length==2)? fileNameFromHeader[1]: fileName
