    ...
    public MLWorkerClient createClient(boolean isInternal, boolean raiseExceptionOnFailure) {
        try {
            ...
            if (raiseExceptionOnFailure) {
                ...
                String fix = isInternal ? "docker-compose up -d ml-worker" : "<giskard worker start -h GISKARD_ADDRESS> in the terminal of the machine that execute your model. For help please refer to the doc: https://docs.giskard.ai/start/guides/installation/ml-worker";
                throw new GiskardRuntimeException(String.format("Failed to establish a connection with %s ML Worker.\nStart it by running %s", workerType, fix), e);
            }
            return null;
        }
    }
