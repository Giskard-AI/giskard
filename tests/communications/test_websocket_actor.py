...
def test_websocket_actor_run_model_for_data_frame_regression(request):
    ...
    with utils.MockedProjectCacheDir(), utils.MockedClient(mock_all=False) as (client, mr):
        ...
        # client
        reply = listener.run_model_for_data_frame(client=client, params=params)
...
def test_websocket_actor_run_model_for_data_frame_classification(request):
...
def test_websocket_actor_explain_text_ws_regression(request):
    dataset: Dataset = request.getfixturevalue("hotel_text_data")
    model: BaseModel = request.getfixturevalue("hotel_text_model")
    ...
    with utils.MockedProjectCacheDir(), utils.MockedClient(mock_all=False) as (client, mr):
        ...
        reply = listener.explain_text_ws(client=client, params=params)
...
def test_websocket_actor_explain_text_ws_not_text(request):
    ...
    with utils.MockedProjectCacheDir(), utils.MockedClient(mock_all=False) as (client, mr):
        ...
        with pytest.raises(ValueError, match="Column .* is not of type text"):
            listener.explain_text_ws(client=client, params=params)


def test_websocket_actor_explain_text_ws_classification(request):
    dataset: Dataset = request.getfixturevalue("enron_data")
    model: BaseModel = request.getfixturevalue("enron_model")
...
def test_websocket_actor_dataset_processing_empty(request):
    dataset: Dataset = request.getfixturevalue("enron_data")
