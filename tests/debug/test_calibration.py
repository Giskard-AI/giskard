...
def test_confidence(model, dataset, request):
    ...
    overconfidence_predictions = model.predict(
        dataset.slice(lambda x: x.loc[result_overconfidence.failed_indexes], row_level=False)
    )

    rest_dataset_overconfidence = dataset.copy()
    rest_dataset_overconfidence.df = rest_dataset_overconfidence.df.drop(result_overconfidence.failed_indexes)
    rest_predictions_overconfidence = model.predict(rest_dataset_overconfidence)

    assert len(result_overconfidence.failed_indexes) == 59
    ...
    underconfidence_predictions = model.predict(
        dataset.slice(lambda x: x.loc[result_underconfidence.failed_indexes], row_level=False)
    )

    rest_dataset_underconfidence = dataset.copy()
    rest_dataset_underconfidence.df = rest_dataset_underconfidence.df.drop(result_underconfidence.failed_indexes)
    rest_predictions_underconfidence = model.predict(rest_dataset_underconfidence)

    assert len(result_underconfidence.failed_indexes) == 45
