...
class MetamorphicTests(AbstractTestCollection):
    @staticmethod
    def _perturb_and_predict(df, model: ModelInspector, perturbation_dict, classification_label_index=None):
        ...
        if len(modified_rows):
            results_df["perturbed_prediction"] = run_predict(df.loc[modified_rows], model).raw_prediction
        else:
    ...
    def test_metamorphic_decreasing(self,
                                    ...
                                    model,
                                    perturbation_percent: float,
                                    classification_label_index: int = None,
                                    failed_threshold=1):
