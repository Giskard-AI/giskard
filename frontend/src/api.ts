...
import {apiUrl, apiUrl2} from '@/env';
import {
...
export const api = {
    ...
    async logInGetToken(username: string, password: string) {
        return axios.post(`${apiUrl}/api/authenticate`, {
            'username': username,
            'password': password
        });
    },
    ...
    async getRoles(token: string) {
        return axios.get<string[]>(`${apiUrl}/api/v1/roles/`, authHeaders(token));
    },
    async updateUser(token: string, data: IUserProfile) {
        return axios.put(`${apiUrl}/api/admin/users`, data, authHeaders(token));
    },

    async createUser(token: string, data: IUserProfileCreate) {
        return axios.post(`${apiUrl}/api/admin/users/`, data, authHeaders(token));
    },
    async signupUser(userData: IUserProfileCreate, token: string) {
        return axios.post(`${apiUrl}/api/v1/users/open`, {...userData, token});
    },
    // migrated
    async deleteUser(token: string, login: string) {
        return axios.delete(`${apiUrl}/api/admin/users/${login}`, authHeaders(token));
    },
