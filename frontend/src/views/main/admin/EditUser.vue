          ...
          <v-card-text>
            <v-row>
              ...
              <v-col cols=6>
                ...
                <v-select label="Role" v-model="roleId" :items="roles" :error-messages="errors"></v-select>
                </ValidationProvider>
            ...
            <v-row class="mt-0">
              <v-col cols=6>
                <ValidationProvider name="Password" mode="eager" rules="password" v-slot="{errors}">
                  <v-text-field dense type="password" autocomplete="new-password" ref="password" label="Password" v-model="password1" :error-messages="errors" 
                  :disabled="!setPassword"></v-text-field>
                </ValidationProvider>
              </v-col>
              <v-col cols=6>
                <ValidationProvider name="Password confirmation" mode="eager" rules="confirms:@Password" v-slot="{errors}">
                  <v-text-field dense type="password" autocomplete="new-password" label="Confirm password" v-model="password2" :error-messages="errors"
                  :disabled="!setPassword"></v-text-field>
                </ValidationProvider>
              </v-col>
            </v-row>
...
export default class EditUser extends Vue {
  ...
  get roles() {
    return readAdminRoles(this.$store).map(e => {
      return {
        value: e,
        text: Role.rolesById[e]
      }
    });
  }
  ...
  public submit() {
    this.$refs.observer.validate().then(async () => {
      ...
      // }
      // updatedProfile.roles = [this.roleId];
