...
class TextTypoTransformation(TextTransformation):
    ...
    def __init__(self, column: str, rate: float = 0.05, min_length: int = 10, rng_seed: int = 1729) -> None:
    ...
    def _random_key_typo(self, char: str) -> str:
...
class TextAccentRemovalTransformation(TextTransformation):
    ...
    def __init__(self, column: str, rate: float = 1.0, rng_seed: int = 1729) -> None:
        super().__init__(column)
        self.rate = rate
        self.rng = np.random.default_rng(seed=rng_seed)
...
class TextLanguageBasedTransformation(TextTransformation):
    def __init__(self, column: str, rng_seed: int = 1729) -> None:
        super().__init__(column, needs_dataset=True)
        self._lang_dictionary: dict = dict()
...
class TextNumberToWordTransformation(TextLanguageBasedTransformation):
    ...
    def make_perturbation(self, row: pd.Series) -> str:
        ...
        if lang == "fa" or lang == "id":
            # In num2words, the converter of "fa" and "id" are buggy,
...
class TextFromSpeechTypoTransformation(TextLanguageBasedTransformation):
    ...
    def __init__(self, column: str, rng_seed: int = 1729, min_length: int = 10) -> None:
