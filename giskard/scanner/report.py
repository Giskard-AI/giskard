...
class ScanReport:
    ...
    def generate_test_suite(self, name=None):
        ...
        if self.model:
            ...
            placeholder_params.update({"model": ModelInput("model")})
        if self.dataset:
            suite_default_params.update({"dataset": self.dataset})
            placeholder_params.update({"dataset": DatasetInput("dataset")})
        ...
        for test, test_name in self.generate_tests(with_names=True):
            # Test needs a suite input to be replaced during execution
            # FIXME: What happens if params are added without such params
            suite.add_test(test, test_name, test_name, **placeholder_params)
