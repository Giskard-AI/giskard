...
from giskard.core.suite import ModelInput, DatasetInput
...
class ScanReport:
    def __init__(self, issues: Sequence[Issue], as_html: bool = True):
        self.issues = issues
    ...
    def generate_tests(self, with_names=False, overwrite_inputs={}):
        tests = sum([issue.generate_tests(with_names=with_names, overwrite_inputs=overwrite_inputs) for issue in self.issues], [])
        return tests

    def generate_test_suite(self, name=None, use_suite_input=True) -> giskard.Suite:
        suite = giskard.Suite(name=name or "Test suite (generated by automatic scan)")
        # Create model and dataset as suite-level inputs for overwritting
        suite_inputs = {
            "model": ModelInput("model"),
            "dataset": DatasetInput("dataset"),
        } if use_suite_input else {}
        for test, test_name in self.generate_tests(with_names=True, overwrite_inputs=suite_inputs):
